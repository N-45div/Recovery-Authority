...
describe("filesystem recovery authority", () => {
  test("proves, commits, and recovers a directory delete", async () => {
    ...
    const committed = await service.commit(prepared.operation.id, prepared.capability);
  ...
  test("rejects commit when state changes after proof", async () => {
    ...
    expect(service.commit(prepared.operation.id, prepared.capability)).rejects.toThrow(
