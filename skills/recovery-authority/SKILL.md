...
Route destructive filesystem deletes, local SQLite mutations, and Git hard resets through the Recovery Authority MCP tools.
...
4. For `git reset --hard`, call `recovery_prepare_git_reset_hard` with the repository root and exact target.
5. Report the operation ID, affected scope, proof digest, expiry, and restore-drill result.
6. Commit only through the corresponding recovery commit tool while the capability remains fresh.
7. Call `recovery_get_operation` when status is uncertain and the corresponding restore tool when the effect must be reversed.
8. For other Git commands, remote databases, infrastructure, overwrite, mixed-effect, or opaque-script findings, stop and explain that one exact recovery adapter does not cover the operation.

## Boundaries

- Only `filesystem.delete`, local `sqlite.mutate`, and `git.reset-hard` currently have exact recovery adapters. Other categories are block-only.
